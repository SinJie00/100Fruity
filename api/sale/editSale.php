<?php
    ...
    function editSale($saleID,$itemNumber, $customerID,$customerName,$itemName, $saleDate, $discount, $quantity,$unitPrice){
        $conn = new PDO(DSN, DB_USER, DB_PASSWORD);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        if(isset($saleID)){

            $saleDetailsItemNumber = $itemNumber;
            $saleDetailsSaleDate = $saleDate;
            $saleDetailsItemName = $itemName;
            $saleDetailsQuantity = $quantity;
            $saleDetailsUnitPrice = $unitPrice;
            $saleDetailsSaleID = $saleID;
            $saleDetailsCustomerName = $customerName;
            $saleDetailsDiscount = $discount;
            $saleDetailsCustomerID = $customerID;
            ...
            if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
                ...
                if($originalSaleQuantityStatement->rowCount() > 0){
                    ...
                    if($originalOrderItemNumber !== $saleDetailsItemNumber) {
                        ...
                        $newItemCurrentStockSql = 'SELECT * FROM fruit WHERE id = :id';
                        ...
                        $originalQuantityForNewItem = $newItemRow['amount'];
                        ...
                        $newItemStockUpdateSql = 'UPDATE fruit SET amount = :amount WHERE id = :id';
                        $newItemStockUpdateStatement = $conn->prepare($newItemStockUpdateSql);
                        $newItemStockUpdateStatement->execute(['amount' => $newItemNewStock, 'id' => $saleDetailsItemNumber]);
                        
                        // Get the current stock of the previous item
                        $previousItemCurrentStockSql = 'SELECT * FROM fruit WHERE id=:id';
                        $previousItemCurrentStockStatement = $conn->prepare($previousItemCurrentStockSql);
                        $previousItemCurrentStockStatement->execute(['id' => $originalOrderItemNumber]);
                        ...
                        $currentQuantityForPreviousItem = $previousItemRow['amount'];
                        ...
                        $previousItemStockUpdateSql = 'UPDATE fruit SET amount = :amount WHERE id = :id';
                        $previousItemStockUpdateStatement = $conn->prepare($previousItemStockUpdateSql);
                        $previousItemStockUpdateStatement->execute(['amount' => $previousItemNewStock, 'id' => $originalOrderItemNumber]);
                        ...
                        $stockSql = 'SELECT * FROM fruit WHERE id=:id';
                        $stockStatement = $conn->prepare($stockSql);
                        $stockStatement->execute(['id' => $saleDetailsItemNumber]);
