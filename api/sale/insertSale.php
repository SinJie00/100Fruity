<?php
	...
	function createSale($itemNumber, $customerID,$customerName,$itemName, $saleDate, $discount, $quantity,$unitPrice){
		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			$stockSql = 'SELECT amount FROM fruit WHERE id = :id';
			$stockStatement = $conn->prepare($stockSql);
			$stockStatement->execute(['id' => $itemNumber]);
			if($stockStatement->rowCount() > 0){
				// Item exits in DB, therefore, can proceed to a sale
				$row = $stockStatement->fetch(PDO::FETCH_ASSOC);
				$currentQuantityInItemsTable = $row['amount'];
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						...
						$stockUpdateSql = 'UPDATE fruit SET amount = :amount WHERE id = :id';
						$stockUpdateStatement = $conn->prepare($stockUpdateSql);
						$stockUpdateStatement->execute(['amount' => $newQuantity, 'id' => $itemNumber]);
