<?php
	...
	if(isset($_POST['fruitImageItemNumber'])){
		
		$itemImageItemNumber = htmlentities($_POST['fruitImageItemNumber']);
		...
		if(!empty($itemImageItemNumber)){
			...
			if($_FILES['fruitImageFile']['name'] != ''){
				...
				$itemNumberSql = 'SELECT * FROM fruit WHERE id = :itemNumber';
				...
				if($itemNumberStatement->rowCount() > 0){
					...
					$arr = explode('.', $_FILES['fruitImageFile']['name']);
					...
					if(in_array($extension, $allowedTypes)){
						...
						$baseImageFolder = '../../data/item_images/';
						$itemImageFolder = '';
						$fileName = time() . '_' . basename($_FILES['fruitImageFile']['name']);
						...
						if(move_uploaded_file($_FILES['fruitImageFile']['tmp_name'], $targetPath)){
							
							// Update image url in item table
							$updateImageUrlSql = 'UPDATE fruit SET imageURL = :imageURL WHERE id = :itemNumber';
